...
namespace WebApp.Areas.CRM.Controllers
{
    ...
    public class ContactController : AppController
    {
        ...
        public ActionResult Summary(Guid Id)
        {
            ...
            model.SelectedContactTypes = cntct.GetByContact(model.Id);
            ViewBag.SalesGrid = "0"; //orders.GetSaleOrders((int)OrderType.Sale, (int)OrderType.SalesReturn, (int)OrderType.SalesExchange).Where(x => x.ContactId == Id).ToList();
            ViewBag.POList = "0"; //orders.GetPOList((int)OrderType.Purchase).Where(x => x.ContactId == Id).ToList();
            ViewBag.OrderDetails = "0"; //orders.GetOrderDetails();
            return View(model);
        }
        #endregion
